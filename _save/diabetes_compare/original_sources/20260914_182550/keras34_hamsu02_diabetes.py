...
# 28번 추가: scaler는 실제 훈련 데이터에만 fit한다.
scaler = RobustScaler()
...
# 34번 추가: 층/초기값/Dropout 비율은 33번 그대로, 연결 문법만 바꾼다.
input1 = Input(shape=(10,))
dense1 = Dense(3, activation='relu', name='dense1', kernel_initializer=GlorotUniform(seed=SEED + 1))(input1)
drop1 = Dropout(0.2, seed=SEED + 101)(dense1)
dense2 = Dense(10, activation='relu', name='dense2', kernel_initializer=GlorotUniform(seed=SEED + 2))(drop1)
drop2 = Dropout(0.3, seed=SEED + 102)(dense2)
dense3 = Dense(15, activation='relu', name='dense3', kernel_initializer=GlorotUniform(seed=SEED + 3))(drop2)
drop3 = Dropout(0.5, seed=SEED + 103)(dense3)
dense4 = Dense(20, activation='relu', name='dense4', kernel_initializer=GlorotUniform(seed=SEED + 4))(drop3)
dense5 = Dense(10, activation='relu', name='dense5', kernel_initializer=GlorotUniform(seed=SEED + 5))(dense4)
dense6 = Dense(1, name='dense6', kernel_initializer=GlorotUniform(seed=SEED + 6))(dense5)
...
# 31번 추가: 단계별로 다른 이름에 저장하여 이전 실험을 덮어쓰지 않는다.
checkpoint_path = result_dir / 'keras34_hamsu02_diabetes.keras'
mcp = ModelCheckpoint(checkpoint_path, monitor='val_loss', save_best_only=True)
joblib.dump(scaler, result_dir / 'keras34_hamsu02_diabetes_scaler.joblib')
...
save_result(__file__, result_dir, config, hist, loss, y_test, y_predict,
            seconds, initial_hash, previous='keras33_dropout02_diabetes', plot=True)
