# [실습] 가중치만 저장하기 - save_weights
#
# model.save()          → 구조 + 가중치를 모두 저장 (.keras)
# model.save_weights()  → 가중치(w, b)만 저장      (.weights.h5)
#
# 가중치만 저장하면 파일이 작지만, 나중에 불러올 때
# 저장할 때와 똑같은 모델 구조를 먼저 만들어놓아야 한다.
# 구조가 다르면 가중치의 모양이 안 맞아서 에러가 난다.
#29-3카피
