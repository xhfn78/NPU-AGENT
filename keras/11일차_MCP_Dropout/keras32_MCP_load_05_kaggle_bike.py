# [실습] ModelCheckpoint 불러오기 - 캐글 자전거 대여량 (회귀)
#
# keras31_MCP_save_05 가 저장해둔 체크포인트를 불러와서 평가만 한다.
# 먼저 keras31_MCP_save_05 를 실행해서 저장 파일을 만들어야 동작한다.
...
files = sorted(Path('./_save/keras30').glob('k31_05_*.keras'), key=lambda file: file.stat().st_mtime)
if not files:
    raise FileNotFoundError('k31_05 체크포인트가 없습니다. keras31_MCP_save_05를 먼저 실행하세요.')
