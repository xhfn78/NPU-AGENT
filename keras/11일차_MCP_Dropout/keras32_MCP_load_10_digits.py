# [실습] ModelCheckpoint 불러오기 - 손글씨 숫자 (다중 분류)
#
# keras31_MCP_save_10 가 저장해둔 체크포인트를 불러와서 평가만 한다.
# 먼저 keras31_MCP_save_10 를 실행해서 저장 파일을 만들어야 동작한다.
...
files = sorted(Path('./_save/keras30').glob('k31_10_*.keras'), key=lambda file: file.stat().st_mtime)
if not files:
    raise FileNotFoundError('k31_10 체크포인트가 없습니다. keras31_MCP_save_10를 먼저 실행하세요.')
